            ...
            try:
                ...
                for _, g in df.groupby('rid'):
                    ...
                    for i in range(len(g) - 1):
                        # Features: current delay, stops remaining, current stop index
...
def find_station(text):
    ...
    if "LONDON" in text and "BRIDGE" not in text:
...
def booking_flow():
    ...
    if input("\nDo you need a return? [y/N]: ").lower() == 'y':
        ...
        if r_date and r_time:
            ...
            print("Bot: Invalid return format.")
...
def delay_flow():
    ...
    # Inputs for feature calculation
    d = parser.num(input(f"Minutes delay currently reported at {curr_n}: "))
    total_stops = parser.num(input("Total stops for this journey: "))
    curr_pos = parser.num(input(f"Stop sequence number for {curr_n} (e.g. 1): "))
    sched = parser.time(input(f"Scheduled arrival at {dest_n} (HH:MM): "))

    if sched:
        # Calculate stops left for the model
        stops_left = max(0, total_stops - curr_pos)
        
        # Get Knowledge-based prediction
        y_pred = predictor.predict(d, stops_left, curr_pos)
        
        # Calculate time
        new_arrival = datetime.strptime(sched, "%H:%M") + timedelta(minutes=y_pred)

        print("\n" + "="*35)
        print(f" KNOWLEDGE-BASED JOURNEY ANALYSIS")
        print("-" * 35)
        print(f"Current Position  : Stop {curr_pos} of {total_stops}")
        print(f"Stops Remaining   : {stops_left}")
        print(f"Current Delay     : {d} min")
        print("-" * 35)
        print(f"Predicted Target(y): {y_pred} min delay at end")
        print(f"Expected Arrival   : {new_arrival.strftime('%H:%M')}")
        print("="*35)
    else:
